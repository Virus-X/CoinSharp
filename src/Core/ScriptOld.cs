...
namespace CoinSharp
{
    ...
    public class ScriptOld
    {
        ...
        /// </summary>
        /// <param name="networkParams">Network parameters.</param>
        ...
        public ScriptOld(NetworkParameters networkParams, byte[] programBytes, int offset, int length)
        {
        ...
        internal static void WriteBytes(Stream os, byte[] buf)
        {
            ...
            {
                os.WriteByte(OpPushData1);
                os.WriteByte((byte)buf.Length);
                os.Write(buf);
            }
            else if (buf.Length < 65536)
            {
                os.WriteByte(OpPushData2);
                os.WriteByte((byte)buf.Length);
        ...
        internal static byte[] CreateOutputScript(Address to)
        {
            using (var bits = new MemoryStream())
            {
                ...
                bits.WriteByte(OpDup);
                bits.WriteByte(OpHash160);
                WriteBytes(bits, to.Hash160);
                bits.WriteByte(OpEqualVerify);
                bits.WriteByte(OpCheckSig);
                return bits.ToArray();
            }
        }
